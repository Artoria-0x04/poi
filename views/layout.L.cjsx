...
$('#layout-css').setAttribute 'href', "./assets/css/layout.L.css"
factor = null
...
adjustSize = ->
  ...
  """
  $('kan-game #webview-wrapper')?.style?.height = $('kan-game webview')?.style?.height = $('kan-game webview /deep/ object[is=browserplugin]')?.style?.height = "#{webviewHeight - 1}px"
  $('kan-game').style.marginTop = "0px"
if !window._delay
  adjustSize()
else
...
handleResize = ->
  {webviewWidth} = window
  webviewHeight = Math.floor(webviewWidth / 10 * 6)
  if webviewWidth != -1
  ...
  if !window._delay
    ...
    window._delay = false
  if window.innerWidth < webviewWidth
    window.innerWidth = webviewWidth
  if window.innerHeight < webviewHeight
    window.innerHeight = webviewHeight
